...
<div id="register" class="auto">
    <h2>欢迎注册</h2>
    <form action="{{route('front.regist')}}" method="post" >
        @if (count($errors) > 0)
        ...
        <label>用户名：<input type="text" name="user" /><span>*用户名禁止特殊使用字符，</span></label>
        <label>密码：<input type="password" name="password" /><span>*密码含禁止使用特殊用字符</span></label>
            <label>确认密码：<input type="password" name="password_confirmation" /><span>*密码含禁止使用特殊用字符</span></label>
        ...
        <input class="btn" type="submit" name="login" value="注册" />
    </form>
</div>
